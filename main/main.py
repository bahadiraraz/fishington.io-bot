...
while True:
    ...
    for pic, contour in enumerate(countours):
        ...
        if 600 > area2 > 100:
            ...
            x_red2 = int(x1 + w1 / 2)
            y_red2 = int(y1 + h1 / 2)
            print("lolllllllllllllll",abs(acceleration_red_bar))
            ...
            try:
                ...
                cv2.putText(frame, "green bar: " + str(x_green) + " " + str(y_green), (10, 20), cv2.FONT_HERSHEY_SIMPLEX,
                            0.7, (0, 0, 255))
                if not np.array_equal(frame_red, frame_green) and distance > 30:
                    if x_green > x_red2 and (x_red2 < x_red1):
                        print("red bar on the left1")
                        mouse.press(Button.left)
                    elif x_green < x_red2 and (x_red2 > x_red1)and distance > 30:
                        mouse.release(Button.left)
                        print("red bar on the left2")

                else:
                        if np.array_equal(frame_red, frame_green)  and distance > distance2:
